...
local Table = require("scripts.TPABOBAP.AlchemyRedone.old.ui.item_table")
        ...
        content = ui.content {
            {
                ...
                props = {
                    text = '*',
                    textSize = T.Base.TEXT_SIZE_CONTENT * 2,
...
m.makeIngredientTable = function(wnd)
    ...
    ---@type AlchemyData
    local data = wnd.data
    local rowHeight = 1.5 * (T.Base.TEXT_SIZE_CONTENT + 2)
    local effectWidth = 4 * (T.Base.TEXT_SIZE_CONTENT + 3)

    local function renderEffects(ingredient, width, height)
        local record = types.Ingredient.record(ingredient.id)
        local effects = record and record.effects or {}
        local sz = T.Base.TEXT_SIZE_CONTENT
